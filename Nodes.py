...
class GenerateNodes:
    ...
    def __init__(self, book, chapter, label='trans'):
        self.book = book
    ...
    def Label(self, word):
        '''
        Input: word node
        Output: actor label in preferred format (translitteration, Hebrew, or gloss)
        '''
        
        if self.label == 'trans':
            lab = F.lex.v(word)
        elif self.label == 'heb':
            lab = F.voc_lex_utf8.v(word)
        elif self.label == 'gloss':
            lab = F.gloss.v(L.u(word, 'lex')[0])
        ...
        for n in self.nodeList():
            if not E.coref.f(n):
                if F.otype.v(n) in ['phrase_atom']:
                    if F.function.v(L.u(n, 'phrase')[0]) in ['Pred','Subj','Objc','Cmpl','PreS','PreO','PreC','PtcO']:
                        actor_str = ''
                        for w in L.d(n, 'word'):
                    ...
                    if F.prs.v(n) not in ['absent','n/a']:
                        actor_str = f'{F.prs_ps.v(n)[1]}{F.prs_gn.v(n)}{F.prs_nu.v(n)[0]}_sfx'
                        if actor_str:
                            self.checkActorDict(actor_dict, actor_str, [n])
